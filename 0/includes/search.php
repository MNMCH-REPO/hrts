...
try {
    // Base query to fetch tickets submitted by or assigned to the logged-in user
    $query = "
        SELECT 
            t.id, 
        ...
        WHERE t.employee_id = :user_id OR t.assigned_to = :user_id
    ";
    ...
    // Bind parameters
    $stmt->bindParam(':user_id', $userId, PDO::PARAM_INT); // Bind the logged-in user's ID
    if (!empty($search)) {
        $searchParam = "%$search%";
